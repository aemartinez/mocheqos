...
traceOf :: Run -> [Action]
traceOf = removeLoopMsgs . L.map (\(_, (s1, s2, ptp1, ptp2, dir, msg), _) -> ((ptp1, ptp2), dir, msg))
  -- we filter out loop messages produced by the projection algorithm
  where removeLoopMsgs = L.filter (\((ptp1, ptp2), dir, msg) ->  not (isPrefixOf "__l__" msg || isPrefixOf "__e__" msg))

showAction :: Action -> String
showAction ((s,r), d, msg) = s ++ "." ++ r ++ dirMap!d ++ msg
  where dirMap = M.fromList [(Send, "!"), (Receive, "?")]

showTrace :: [Action] -> String
showTrace trace = L.intercalate "; " (L.map showAction trace)
...
getStatesFromRun :: System -> Run -> [(Ptp, State)]
getStatesFromRun sys [] = []
getStatesFromRun sys run =
    L.foldr (\(_, kevt, _) states -> subjectState kevt ++ states) [] run ++ getStatesFromConf sys (gtarget $ last run)
    where subjectState kevt
              | direction kevt == Send    = [(sender kevt , senderState kevt)]
              | direction kevt == Receive = [(receiver kevt, receiverState kevt)]

getStatesFromConf :: System -> Configuration -> [(Ptp, State)]
getStatesFromConf (cfsms, ptps) (states, buf) = [(ptp, states !! id) | (id, ptp) <- M.toList ptps]
