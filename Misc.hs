...
type Graph vertex label =
  (Set vertex, vertex, Set label, Set(Edge vertex label))
...
-- drop the shortest prefix of 'ls' whose elements do not satisfy
...
pairwiseDisjoint f =
-- return the pair of keys (k1,k2) such that intersect (f!k1) (f!k2)
  case M.keys f of
    [] -> Nothing
    [_] -> Nothing
    x:y:_ ->
...
-- POST: return the reflexo-transitive closure of the relation in the 2nd argument
...
-- associate a list of messages to each command; the first string of the list
...
info =
  ...
  (GC2DOT, ["return the dot format of a g-choreography",
            "[-v] [-d dirpath] [-fmt (gml | gc | gmldiff | sloppygml)] [-e] [-o destfile] filename",
            "\t-v diplays the path to the resulting file; not set by default",
            "default: dirpath = " ++ dirpath,
            "\t-fmt = gc",
            "\t-e do not hide (o) in the 'gc' format"
           ]
  ),
  ...
  (PROJ, ["return projections of a g-choreography",
          "[-D (min | det | no)] [-fmt (fsa | dot | gc | GC)] [-a] [-v] filename -p <blank-separated list of participants>",
            "default: -D no",
            "\t -fmt fsa; the formats 'gc' and 'GC' are for per-participant or collective restriction",
            "\t -a list accepting states",
            "\t -v diplays the mapping index |--> ptp",
            "\t -qos parses qos syntax in messages and returns a qosfsa",
  ...
  (GC2LATEX, ["generate the latex of a g-choreography using some macros defined by eM",
           "[--vfactor f] [--hfactor f] [--vunit unit] [--hunit unit] filename",
            "default: --vfactor factor to scale vspace; default bound = .1",
            "\t --hfactor factor to scale hspace; default bound = .9",
            "\t --vunit latex unit for vertical space; default --vunit pt",
            "\t --hunit latex unit for horizontal space; default --hunit pt"
         ]
  ),
  (GC2SGC, ["generate the latex of a g-choreography using the sgg style of listings",
           "filename"
         ]
  ),
  ...
  (QLSOLVER, ["checks if a QL formula is valid in a QoS Communicating System",
              "qos_system_filename",
              "ql_formula_filename"]
  )
...
defaultFlags cmd =
  M.fromList (case cmd of
                 ...
                 GC2DOT -> [("-d",dirpath), ("-o", ""), ("-v",""), ("-fmt","gc"), ("-e", "")]
                 GC2FSA -> [("-d",dirpath), ("-o", ""), ("-v",""), ("-u", "-1")]
                 PROJ   -> [("-d",dirpath), ("-o", ""), ("-v",""), ("-a","no"), ("-D","no"), ("-fmt", "fsa")]
                 ...
                 WS     -> []
                 WF     -> []
                 GC2LATEX  -> [("--vfactor", ".1"), ("--hfactor", ".9"), ("--vunit", "pt"), ("--hunit", "pt")]
                 GC2SGC -> []
...
getFlags cmd args =
  case cmd of
    ...
    GC2DOT -> case args of
      ...
      "-d":y:xs     -> M.insert "-d" y    (getFlags cmd xs)
      "-e":xs       -> M.insert "-e" "-e" (getFlags cmd xs)
      "-v":xs       -> M.insert "-v" yes  (getFlags cmd xs)
      "-o":y:xs     -> M.insert "-o" y    (getFlags cmd xs)
      _             -> error $ usage(cmd)
    GC2FSA -> case args of
    ...
    PROJ -> case args of
      []            -> defaultFlags(cmd)
      "-v":xs       -> M.insert "-v" yes (getFlags cmd xs)
      "-a":xs       -> M.insert "-a" yes (getFlags cmd xs)
      "-fmt":y:xs  -> M.insert "-fmt" y (getFlags cmd xs)
      "-D":y:xs     -> M.insert "-D" y (getFlags cmd xs)
      "-qos":xs    -> M.insert "-qos" yes (getFlags cmd xs)
      _             -> error $ usage(cmd)
    UNFOLD -> case args of
      []            -> defaultFlags(cmd)
      "-v":xs       -> M.insert "-v" yes (getFlags cmd xs)
...
pClosure g lpred v =
--  PRE:
--  POST: return the closure of vertexes reachable from v with
...
-- return the first set in 'classes' containing 'state'
...
--  POST: return the outgoing edges of v in gr
...
-- gincoming gr
--  PRE:  
--  POST: build a map associating to each vertex of gr its incoming edges
...
--   POST: return the list of (elementary) paths in gr from s to t not passing trough veterexes in l
gpath gr s t l
    | s == t    = if (s € l)
                  then []
