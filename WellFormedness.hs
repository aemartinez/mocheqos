...
import PomsetSemantics (labelOf, eventsOf, orderOf, seqLeq, pomsetsOf)
...
interactionsOf gc =
-- return the set of interactions occurring in gc
  case gc of
    Emp -> S.empty
...
sender :: GC -> Ptp
-- return the sender of an interaction
sender (Act (s, _) _) = s
...
filterPtp :: Ptp -> GC -> GC
filterPtp p gc =
  -- project gc on p...without splitting interactions
  case gc of
    Emp -> Emp
    Act (s,r) _ ->
      if p == s || p == r
...
firstOnly :: GC -> GC
firstOnly gc =
  -- return a g-choreography with the same "top-level" structure of
...
lastOnly :: GC -> GC
lastOnly gc =
-- return a g-choreography with the same "bottom-level"
...
cutFirst gc =
-- separate the first interactions of gc from the rest
...
ws gc =
{-
   check for well-sequencedness and return a g-choreography that
