...
instance Hashable Dir where
    ...
    hashWithSalt s dir = case dir of 
      Send    -> hashWithSalt s (0::Int)
      Receive -> hashWithSalt s (1::Int)
      Tau     -> hashWithSalt s (2::Int)
      Break   -> hashWithSalt s (3::Int)
      LoopSnd -> hashWithSalt s (4::Int)
      LoopRcv -> hashWithSalt s (5::Int)
...
isAccept :: Action -> Bool
isAccept (_, d, _) = (d == Accept)

isCommunication :: Action -> Bool
isCommunication a = (isSend a) || (isReceive a) || (isAccept a)
...
acceptingTrx :: Ptp -> State -> LTrans
acceptingTrx p q = (q, ((p,p), Accept, ""), q)

isAccepting :: LTrans -> Bool
isAccepting (_, (_,d,_),_) =
  d == Accept

acceptingOf :: CFSM -> Set State
acceptingOf (states,_,_,trxs) =
  S.map (\(q,_,_) -> show $ S.findIndex q states) (S.filter isAccepting trxs)
...
existSend set =
  F.or $ S.map (\( _, dir, _ ) -> dir == Send || dir == LoopSnd) set
...
showDir d flines
    ...
    | d == Tau       = " " ++ flines!tau ++ " "
    | d == Break     = " " ++ flines!breakExc ++ " "
    | d == Accept    = " " ++ flines!acpExc ++ " "
...
finalStates :: CFSM -> Set State
finalStates cfsm =
  S.filter aux (statesOf cfsm)
  where
    aux q =
      S.null (goutgoing cfsm q)

replaceState :: State -> State -> CFSM -> CFSM
replaceState q q' m =
-- replace q with q' in m (substitution q |--> q')
  replaceVertex q q' m

replaceStates :: (State -> Bool) -> State -> CFSM -> CFSM
replaceStates cond q m =
-- replace each state satisfying cond with q in m
  replaceVertexes cond q m
...
-- strToAction _ _ [] =
--   []
-- strToAction ptps sbj [s] =
--   [( ((ptps!sbj), (ptps!sbj) ), Tau, "not action: " ++ s)]
-- strToAction ptps sbj [s,"break"] =
--   [( (ptps!sbj, ptps!sbj ), BreakLoop, "break: " ++ s)]
-- strToAction ptps sbj [s,s'] =
--   [( (ptps!sbj, ptps!sbj ), Tau, "not action: " ++ s ++ " - " ++ s')]
-- strToAction ptps sbj (p:d:msg:xs)
-- -- TODO: what about LoopSnd and LoopRcv?
--   | d == "!" =
--     ( (ptps!sbj, ptps!(read p :: Id)), Send,      msg ):(strToAction ptps sbj xs)
--   | d == "?" =
--     ( (ptps!(read p :: Id), ptps!sbj), Receive,   msg ):(strToAction ptps sbj xs)
--   | d == "tau" =
--     ( (ptps!sbj, ptps!(read p :: Id)), Tau,       msg ):(strToAction ptps sbj xs)
--   | d == "break" =
--     ( (ptps!sbj, ptps!(read p :: Id)), BreakLoop, msg ):(strToAction ptps sbj xs)
--   | otherwise =
--     ( (ptps!sbj, ptps!(read p :: Id)), Tau,       msg ++ "unknown direction: " ++ d ):(strToAction ptps sbj xs)
...
parseFSA text =
  ...
  then
    ...
    error ("malformed CFSM: check lines starting with .outputs, .markings, .states, or .end"
           ++ "\nouts\t" ++ show outs
           ++ "\nmarks\t" ++ show marks
           ++ "\nstarts\t" ++ show sts
           ++ "\nends\t" ++ show ends
  ...
  where ptps' = M.fromList pairs
        ...
        names = getNames 0 (L.map (\line -> if tail line == [] then "" else head $ tail line) outs)
          where getNames :: Int -> [String] -> [String]
                getNames _ []     = []
                getNames i (w:os) = (if w == "" then "__M__"++(show i) else w) : (getNames (i+1) os)
                                 ...
                                 _          ->
                                   case tail x of
                                     ["break", q] -> parsing i xs (st', q0, ev', tr') ms
                                       where
                                         st' = S.insert start (S.insert q st)
...
printAction ( (s,r), dir, msg ) flines =
  case dir of
   LoopSnd -> s ++ (printMessage msg)
   LoopRcv -> r ++ (printMessage msg)
...
dottifyCFSM m p l flines =
  if p == ""
  then ""
  else "digraph " ++ p ++ "{\n" ++ (printCFSM m p flines) ++ l ++ "\n}"
...
dottifySystem flines (cfsms, ptps) =
  ...
  where helper i (x:xs) =
          ...
          then ""
            ...
            let
              header =
                "   subgraph cluster_" ++ (ptps!i) ++ "{\n   label = " ++ (ptps!i) ++ ";\n"
              footer =
                "   }\n"
              cfsm =
                printCFSM x (ptps!i) flines
            in
              (header ++ cfsm ++ footer) ++ "\n" ++ (helper (i+1) xs)
        helper _ [] = ""
    ...
    finish
        where tx =
              ...
              test =
                ...
                "\n.end\n\n\n"
              finish =
                if S.member (ginitialnode m) qs
                then test
                else (show $ ginitialnode m) ++ (show qs)
              prt t =
              ...
              lab ( ( s, r ), dir, msg ) =
                case dir of
                  ...
                  Break   ->
                    if sbj == s
                    then show r ++ " ! " ++ show msg
                    else show s ++ " ? " ++ show msg
                  LoopSnd -> show r ++ " ! " ++ (unpack $ replace (pack "__q") (pack $ "__" ++ r) (pack msg))
                  LoopRcv -> show s ++ " ? " ++ (unpack $ replace (pack "__q") (pack $ "__" ++ s) (pack msg))


cfsm2fsm :: Map String String -> CFSM -> (String, Map State Int)
cfsm2fsm flines m@(states, initn, _, trans) =
  {-
    Transforms a cfsm into the fsm format of mcrl2; the result is a
    string for the fsm format and a map
  -}
  let
    env =
      (M.fromList $ snd $ mapAccumL (\x y -> (x+1,(y,x))) 1 (S.toList states)) :: Map State Int
    liststates = 
...
getAcceptingStates :: System -> String
getAcceptingStates (cfsms, ptps) =
  let
    aux i =
      (ptps!i) ++ ": [" ++ ( L.intercalate "," (S.toList $ acceptingOf (cfsms!!i)) ) ++ "]" 
  in
    "\n\nfinal_states{\n\t" ++
    (L.intercalate ",\n\t" [ aux i | i <- (range $ L.length cfsms), ptps!i /= ""]) ++
    "\n}"
...
removeAcceptingTrxs :: CFSM -> CFSM
removeAcceptingTrxs (states, q0, acts, trxs) =
  (states, q0, acts, S.filter (\(_,act,_) -> not (isAccept act)) trxs)