        ...
        case ggs of
          ...
          g':ggs' -> if S.null p' 
            ...
            where (p', e') = pomsetsOf g' iter e
                  ...
                  --                           _                -> False
                  --            ) (sprod events events')
      Rep _ gg' -> pomsetsOf (unfoldRepBody iter gg') iter e

isTraceInPomsetsPrefix :: Trace -> Set Pomset -> Bool
isTraceInPomsetsPrefix trace = any (isTraceInPomsetPrefix trace)

maxLengthOfLanguage :: Set Pomset -> Int
maxLengthOfLanguage pomsets = S.findMax sizes
  where sizes = S.map (\(evts,_,_) -> S.size evts) pomsets

minLengthOfLanguage :: Set Pomset -> Int
minLengthOfLanguage pomsets = S.findMin sizes
  where sizes = S.map (\(evts,_,_) -> S.size evts) pomsets

isTraceInPomset :: Trace -> Pomset -> Bool
isTraceInPomset trace (events, rel, lab) = isPermutationOfEvents trace (L.map evtToAct $ S.elems events) &&
                                           any (\x -> doesPathExist x (S.delete x events) (updatedEdges x) trace evtToAct) sources
  where evtToAct evt = case M.lookup evt lab of
                            Just x -> x
                            Nothing -> error "No such event in the labeling Map"
        actRel = S.map (bimap evtToAct evtToAct) rel
        sources = minimalEvents events rel
        updatedEdges evt = S.filter (\(x,y) -> x /= evt) rel

isTraceInPomsetPrefix :: Trace -> Pomset -> Bool
isTraceInPomsetPrefix trace (events, rel, lab) = any (\x -> doesPathExist x (S.delete x events) (updatedEdges x) trace evtToAct) sources
  where evtToAct evt = case M.lookup evt lab of
                            Just x -> x
                            Nothing -> error "No such event in the labeling Map"
        sources = minimalEvents events rel
...
isPermutationOfEvents :: Trace -> [Action] -> Bool
isPermutationOfEvents trace actions = sort trace == sort actions

minimalEvents :: Set Event -> Set (Event, Event) -> Set Event
minimalEvents events edges =
  let incoming = S.map snd edges
  in S.difference events incoming

doesPathExist :: Event -> Set Event -> Set (Event, Event) -> [Action] -> (Event -> Action) -> Bool
doesPathExist start events edges [] labels = True
doesPathExist start events edges [a] labels = labels start == a
doesPathExist start events edges (a1:a2:as) labels =
  labels start == a1 &&
      any (\x -> labels x == a2 &&
                  doesPathExist x (S.delete x events) (updatedEdges x) (a2:as) labels
        ) minEvts
      where
        minEvts = minimalEvents events edges
        updatedEdges evt = S.filter (\(x,y) -> x /= evt) edges
